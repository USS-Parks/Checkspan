...
| M3 Multi-agent RAG and software pilot | CS-15–CS-24 → CS-R01–CS-R12 → CS-25 | Approved ("Run M3 STS now please", 2026-09-06); signer, corpus, model, and evaluation-budget stops remain | CS-15–CS-21 complete; local gates passed; CS-22 at the signer stop | Hosted: per-commit runs; recorded at CS-25 |
...
| PC-08 | Undecidable creates a gate without an acceptance cycle | CS-06, CS-21 | CS-06: a gate joined to the node it resolves by an acceptance chain in either direction is rejected at admission (`tests/graph_admission.rs`, local_native passed). CS-21: undecidable and exhausted work opens a complete resolve-work packet in one transaction; the node waits on the packet, never on its own acceptance, and stays gated without a decision across real controller restarts (`tests/gate_packets.rs`, local_native passed). |
