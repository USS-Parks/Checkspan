# Work Graph — schema and workflow notes v0.1

**Paper design, 2026-09-06.** This accompanies the [one-pager](WORK-GRAPH-ONE-PAGER.md). It is a review of the supplied v0 sketch, not an implemented schema, executable plan, or live-system acceptance. Harness remains LOCKED. No Aeneas, Arena, Saddle, or WSF API has been assumed.

## What needed correction

| v0 gap | Consequence | v0.1 change |
| --- | --- | --- |
...
Public-source check: the attribution should be “Prove2Me, as described by Anthropic,” rather than suggesting that it is an Anthropic product. Anthropic describes a theorem DAG and parallel agent work; the platform describes open/proved states and Lean checking. Work Graph’s schema and retry policy below are our proposal, not claims about Prove2Me internals. [Anthropic](https://www.anthropic.com/research/formalizing-fermats-last-theorem), [Prove2Me](https://prove2.me/about).
...
A graph-scoped `wg_<slug>` may remain the local name, but handoffs resolve the full node reference. References to schemas, policies, and verifier code must themselves be immutable or digest-bound. Receipt storage may be a minimal local log for an isolated prototype; an integration must map to the existing authoritative ledger. Neither a new vault nor a new identity system is part of this proposal.
...
wg_ci: check
  deps: exact wg_patch revision / PatchResult@1
  claim: the required trusted checks passed for this candidate
  output: SoftwareCheckResult@1

wg_packet: sink
  deps: exact wg_patch and wg_ci revisions
  claim: a complete packet binds the patch and its required check receipt
  output: ReviewPacket@1

graph.targets: [wg_packet]
```

Here, accepting `wg_patch` establishes artifact integrity only. It cannot satisfy a downstream request for a software-check receipt. The sink requires both the patch and the matching CI result; a green check for another candidate fails the sink contract.
...
If CI rejects a schema-valid patch, retrying `wg_ci` cannot alter the already accepted patch. Repair requires a new patch artifact in a new run (and a new node revision if its contract changes), with fresh downstream bindings. Historical receipts remain attached to the original candidate.

## Paper review cases

These are design walkthroughs and future acceptance cases, not executed tests.

| Case | Required result under v0.1 |
| --- | --- |
...
**Review outcome:** the concept is coherent with these corrections. The supplied v0 remains in the conversation; v0.1 makes its acceptance, retry, and authority boundaries explicit. No runtime, verifier plugin, repository migration, integration, or deployment was performed. The next build decision remains Basho’s; these papers do not authorize execution of a PSPR.
